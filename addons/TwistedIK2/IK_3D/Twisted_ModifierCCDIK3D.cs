...
/// </summary>
[Tool]
public class Twisted_ModifierCCDIK3D : Twisted_Modifier3D
{
    /// <summary>
    ...
    /// <summary>
    /// A NodePath to the Spatial-based node that is used as the tip position
    /// </summary>
    public NodePath path_to_tip = null;
    /// <summary>
    /// A reference to the Spatial-based node that is used as the tip position.
    /// This node should be a child of the last Twisted_Bone3D node in the CCDIK chain for best results.
    /// </summary>
    public Spatial tip_node = null;
    ...
    public override void _Ready()
    {
        ...
        if (path_to_tip != null) {
            tip_node = GetNodeOrNull<Spatial>(path_to_tip);
        }
    }
    ...
    public override bool _Set(string property, object value)
    {
        if (property == "CCDIK/target") {
            ...
            String[] ccdik_data = property.Split("/");
            ...
            if (ccdik_data[3] == "twisted_bone") {
                ...
                current_joint.constraint_angle_min = Mathf.Deg2Rad((float)value);
            }
            else if (ccdik_data[3] == "constraint_angle_max") {
                current_joint.constraint_angle_max = Mathf.Deg2Rad((float)value);
            }
    ...
    public override object _Get(string property)
    {
        if (property == "CCDIK/target") {
            ...
            if (ccdik_data[3] == "twisted_bone") {
                ...
                return Mathf.Rad2Deg(ccdik_joints[joint_index].constraint_angle_min);
            }
            else if (ccdik_data[3] == "constraint_angle_max") {
                return Mathf.Rad2Deg(ccdik_joints[joint_index].constraint_angle_max);
            }
            else if (ccdik_data[3] == "constraint_angle_invert") {
    ...
    private void _ExecuteCCDIKJoint(int joint_index) {
        ...
        Quat end_rotation = bone.Transform.basis.Quat();
        ...
        end_rotation = end_rotation * bone.Transform.basis.Quat();
